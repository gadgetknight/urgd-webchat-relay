...
// Embed an array of texts in one call.
...
// Upsert vectors [{id, values, metadata}] into the Pinecone index.
...
// Lightweight reachability check (control-plane GET, no cost) for the monitor.
async function pineconeReachable() {
  try { const h = await getIndexHost(); return Boolean(h); } catch (_) { return false; }
}

module.exports = { draftAnswer, embedBatch, upsertVectors, pineconeReachable };
