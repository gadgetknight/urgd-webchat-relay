...
//
// CHANGELOG
// 1.2.0  2026-08-24  Added getTicket (full detail + comments) and listTickets.
...
async function getTicket(ticketId) {
  ...
  if (!id) return { ok: false, error: 'bad-ticket-id' };
  const r = await request(`/tickets/${encodeURIComponent(id)}`);
  if (!r.ok) return { ok: false, error: r.error || 'not-found' };
  const payload = r.json;
  const ticket = (payload && payload.ticket && typeof payload.ticket === 'object')
    ? payload.ticket
    : (payload && payload.id ? payload : null);
  if (!ticket) return { ok: false, error: 'not-found' };
...
async function listTickets() {
  const r = await request('/tickets');
  if (!r.ok) return { ok: false, tickets: [], error: r.error };
  return { ok: true, tickets: ticketsFrom(r.json) };
}
